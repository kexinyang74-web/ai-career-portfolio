...
> 先不看书、凭记忆作答。答案批改见《Day3-读懂Git状态-批改版》，指出对错和补充讲解。
...
Changes not staged for commit:
	modified:   总览看板.md
```
1. 嗯，第一行代表他有8个文档在本地没有上传到，就是领先于线上文档，就没有同步的
2. 他第二行意思就是，总览看板这个文件是还没有放在暂存区，就是需要给它施加add命令，然后让让他放到暂存区
**第 2 题**：解释 `git log --oneline` 输出中的三样东西：`(HEAD -> main)`、`(origin/main)`、以及为什么最新提交排在最上面。如果 `origin/main` 停在第 4 行，说明什么？
	 1. `(HEAD -> main)`这个说明我现在正在这个节点上，就是本地在主线上
	 2. (origin/main)这个意思是他那个线上的，就是云端那个仓库，上一次他在那个分支点上
	 3. 以及为什么最新提交排在最上面:这个可能是因为便于查看吧，我觉得
	 4. `origin/main` 停在第 4 行，说明什么？如果它停在第4行，说明本地领先于线上4行的节点，说明有4个接点没有被更新吧，相当于
